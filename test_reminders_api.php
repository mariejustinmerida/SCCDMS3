...
<head>
    ...
    <title>Test Reminders API</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 p-6">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-md p-6">
        <h1 class="text-2xl font-bold mb-4">Reminders API Test</h1>
        ...
        <div class="mb-4">
            ...
            <div class="space-y-4">
                <div>
                    <h3 class="font-medium">Test Get Today's Reminders</h3>
                    <button id="testToday" class="mt-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Run Test</button>
                    <div id="todayResult" class="mt-2 p-3 bg-gray-100 rounded-md overflow-auto max-h-40 hidden"></div>
                </div>
                
                <div>
                    <h3 class="font-medium">Test Get Reminders by Date</h3>
                    <div class="flex items-center mt-2">
                        <input type="date" id="testDate" class="border rounded p-2 mr-2">
                        <button id="testByDate" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Run Test</button>
                    </div>
                    <div id="dateResult" class="mt-2 p-3 bg-gray-100 rounded-md overflow-auto max-h-40 hidden"></div>
                </div>
                
                <div>
                    <h3 class="font-medium">Test Add Reminder</h3>
                    <div class="space-y-2 mt-2">
                        ...
                        <button id="testAdd" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Add Test Reminder</button>
                    </div>
                    <div id="addResult" class="mt-2 p-3 bg-gray-100 rounded-md overflow-auto max-h-40 hidden"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Test Get Today's Reminders
            document.getElementById('testToday').addEventListener('click', function() {
                const resultDiv = document.getElementById('todayResult');
                resultDiv.innerHTML = 'Loading...';
                resultDiv.classList.remove('hidden');
                
                fetch('api/get_todays_reminders.php?test_user_id=1')
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(text => {
                        try {
                            ...
                            resultDiv.innerHTML = `<p class="text-red-600">Error parsing JSON: ${e.message}</p><p>Raw response:</p><pre>${text}</pre>`;
                        }
                    })
                    .catch(error => {
            ...
            // Test Get Reminders by Date
            document.getElementById('testByDate').addEventListener('click', function() {
                const date = document.getElementById('testDate').value;
                const resultDiv = document.getElementById('dateResult');
                resultDiv.innerHTML = 'Loading...';
                resultDiv.classList.remove('hidden');
                
                fetch(`api/get_reminders_by_date.php?date=${date}&test_user_id=1`)
                    .then(response => {
                        if (!response.ok) {
                            throw new Error(`HTTP error! Status: ${response.status}`);
                        }
            ...
            document.getElementById('testAdd').addEventListener('click', function() {
                ...
                fetch('api/reminders.php?test_user_id=1', {
                    method: 'POST',
                    headers: {
